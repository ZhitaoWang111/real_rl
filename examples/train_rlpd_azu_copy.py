...
def actor(agent, data_store, intvn_data_store, env, sampling_rng):
    ...
    # training loop
    timer = Timer()
    running_return = 0.0            # 累计 episode reward
    already_intervened = False      # 是否在干预
    intervention_count = 0          # 干预次数
    intervention_steps = 0          # 干预step数
    ...
    for step in pbar:
        ...
        with timer.context("step_env"):
            ...
            if done or truncated:
                ...
                client.request("send-stats", stats)                     # client.request(type:str, payload:dict) 发布统计数据
                # 更新进度条显示，清空 episode 状态
