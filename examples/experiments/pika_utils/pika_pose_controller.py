...
import threading
import math
from forward_inverse_kinematics import Arm_FK, Arm_IK
import argparse
...
class PikaPoseController:
    ...
    def get_current_joint_positions(self):
        ...
        joint_state = self.robot_interface.piper.GetArmJointMsgs()
    ...
    def start_intervention(self):
        with self.lock:
            if self.current_piper_pose is not None and self.current_pika_pose is not None:
                self.baseline_pose = self.current_piper_pose.copy()
    ...
    def calculate_piper_goal_pose(self):
        ...
        """
        with self.lock:
            if not self.intervention_action or self.baseline_pose is None or self.baseline_pika_pose is None:
                return None
